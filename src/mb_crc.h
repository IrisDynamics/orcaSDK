 ...
 */
 class ModbusCRC {
	...
	static inline constexpr uint8_t crc_hi_table_s[256] = {
	...
	static inline constexpr uint8_t crc_lo_table_s[256] = {
...
public:
	...
	static constexpr uint16_t generate(uint8_t *message, int message_len) {
		...
		while(message_len--) {
			...
			// calculate crc
			index = crc_yte ^ *message++;
			crc_hi_byte = crlo_byte ^ crc_hi_table_s[index];
			crc_lo_byte = crc_lo_table_s[index];
